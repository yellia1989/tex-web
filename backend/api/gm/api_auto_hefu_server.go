...
func AutoHefuServerList(c echo.Context) error {
	...
	sql := "SELECT id,status,src_zoneids,to_zoneid,create_time,hefu_time,start_time,end_time,result FROM t_hefu"
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r.IId, &r.IStatus, &sSrcZoneIds, &r.IMergeToZoneId, &sCreateTime, &sHefuTime, &sStartTime, &sEndTime, &sResult); err != nil {
			return err
		}
...
func AutoHefuServerAdd(c echo.Context) error {
    ...
    now := time.Now().Unix()
    _, err := db.Exec("INSERT INTO t_hefu(src_zoneids, to_zoneid, hefu_time, create_time) VALUES(?,?,?,?)", sSrcZoneIds, iMergeToZoneId, formatTime(parseTime(sHefuTime)), formatTime(now))
	if err != nil {
		return err
	}
...
func AutoHefuServerUpdate(c echo.Context) error {
    ...
    // 正在进行中或已经合服结束的不能修改
	sql := "SELECT id FROM t_hefu WHERE status = 0 and id = " + id
    var total int
    err := db.QueryRow("SELECT count(*) as total FROM ("+sql+") a").Scan(&total)
    if err != nil {
        return err
    ...
    iMergeToZoneId := common.Atoi(sMergeToZoneId)
    _, err = db.Exec("UPDATE t_hefu SET src_zoneids=?,to_zoneid=?,hefu_time=? WHERE id=?", sSrcZoneIds, iMergeToZoneId, formatTime(parseTime(sHefuTime)), id)
	if err != nil {
		return err
	}
