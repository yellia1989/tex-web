...
type GameCrossService struct {
	proxy model.ServicePrxImpl
}

func (s *GameCrossService) SetPrxImpl(impl model.ServicePrxImpl) {
	s.proxy = impl
}
func (s *GameCrossService) SetTimeout(timeout time.Duration) {
	s.proxy.SetTimeout(timeout)
}
func (s *GameCrossService) DoGmCmd(sOperatorName string, sReqCmd string, sResp *string) (int32, error) {
	p := codec.NewPacker()
	var ret int32
	var err error
	var has bool
...
type _GameCrossServiceImpl interface {
	DoGmCmd(ctx context.Context, sOperatorName string, sReqCmd string, sResp *string) (int32, error)
}

func _GameCrossServiceDoGmCmdImpl(ctx context.Context, serviceImpl interface{}, up *codec.UnPacker, p *codec.Packer) error {
	var err error
	var length uint32
	var ty uint32
	var has bool
	impl := serviceImpl.(_GameCrossServiceImpl)
	var p1 string
...
func (s *GameCrossService) Dispatch(ctx context.Context, serviceImpl interface{}, req *protocol.RequestPacket) {
	...
	case "doGmCmd":
		err = _GameCrossServiceDoGmCmdImpl(ctx, serviceImpl, up, p)
		if err != nil {
			break
		}
